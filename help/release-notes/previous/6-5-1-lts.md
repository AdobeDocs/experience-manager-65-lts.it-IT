...
>[!IMPORTANT]
>
> **Hotfix obbligatorio**: per evitare problemi di SNFE (SegmentNotFoundException) con la compattazione offline durante l’installazione del Service Pack 1, installare l’hotfix descritto in [Problemi noti: corruzione dell’archivio durante la compattazione online](#repository-corruption-during-online-compaction-after-offline-compaction-granite-65146).
...
AEM 6.5 Forms LTS su JEE è ora disponibile. Per informazioni dettagliate sugli ambienti supportati, consulta il documento Combinazioni di [piattaforme supportate](/help/forms/using/aem-forms-jee-supported-platforms.md). I collegamenti del programma di installazione sono disponibili nella pagina [Versioni di AEM Forms](https://experienceleague.adobe.com/it/docs/experience-manager-release-information/aem-release-updates/forms-updates/aem-forms-releases).

#### Funzionalità incluse in AEM Forms 6.5 LTS SP1

**Aggiornamenti del supporto Java**

È stato introdotto il supporto per le versioni Java più recenti:

* Oracle Java™ 17/21 (64 bit) (Per ulteriori dettagli, consulta la sezione [Supporto Java](#java-support).

**Aggiornamenti del supporto server applicazioni**

* È stato aggiunto il supporto per JBoss® EAP 8.
* Il framework di sicurezza PicketBox precedente è stato rimosso.
...
AEM Forms su JBoss® EAP 8 utilizza Elytron per la gestione delle credenziali sicure. La clientela deve configurare un archivio credenziali basato su Elytron per garantire l’avvio corretto del server e l’autenticazione sicura del database.
...
* Basato sulla conformità Jakarta EE 9
...
Per ulteriori informazioni, consulta **Migrazione spazio dei nomi da Javax a Jakarta**.

#### Migrazione spazio dei nomi da `javax` a `jakarta`

A partire da **AEM Forms 6.5 LTS SP1**, sono supportati solo i server applicazioni che implementano **API servlet Jakarta 5/6**. Con **Jakarta EE 9 e versioni successive**, tutte le API sono passate dallo spazio dei nomi `javax.{}` a `jakarta.`.
...
**Opzione 1: migrazione codice sorgente (consigliata)**

* Aggiorna tutte le istruzioni di importazione da `javax.{}` a `jakarta.`
* Rigenera e ricompila i progetti con DSC personalizzate
* Implementa nuovamente i componenti aggiornati al server applicazioni
...
**Opzione 2: migrazione binaria utilizzando il trasformatore Eclipse**

* Utilizza lo strumento **trasformatore Eclipse** per convertire i file binari compilati (`.jar`, `.war`) da `javax` in `jakarta`
* Non è richiesta alcuna modifica o ricompilazione del codice sorgente
* Implementa i file binari trasformati nel server applicazioni
...
La tabella seguente mostra le modifiche allo spazio dei nomi comuni richieste durante la migrazione da `javax` a `jakarta`:
...
Utilizza queste mappature come riferimento durante l’aggiornamento del codice sorgente DSC personalizzato o la convalida dei file binari trasformati.
...
>[!NOTE]
>
> (Solo Forms) Oracle Java™ 8/11 (64 bit) richiesto per l’installazione una tantum.
...
**Ambiente**
Si applica a: clientela AEM 6.5 LTS (locale) che installa il Service Pack 1 (SP1). SP1 viene fornito come file Quickstart JAR.
...
* **FORMS-24688:** durante la configurazione di JBoss EAP 8.0 (AEM Forms 6.5.1 LTS) su Linux, gli script creati o modificati in Windows possono contenere terminazioni di riga CRLF, causando errori /bin/sh^M: interprete non valido o $&#39;\r&#39;: comando non trovato. Assicurati che i file interessati utilizzino le terminazioni di riga LF. Questa operazione può essere eseguita utilizzando strumenti quali dos2unix o qualsiasi metodo equivalente.

* **FORMS-24689:** in modalità preconfigurata, l’aggiornamento della porta del database non viene applicato correttamente durante la nuova installazione o l’aggiornamento. In modalità di installazione aggiornata, gli utenti possono selezionare solo tra le porte visualizzate sullo schermo. In modalità di aggiornamento, la nuova porta del database deve essere aggiornata manualmente in lc_turnkey.xml prima di procedere con l’aggiornamento.

* **FORMS-24687:** nelle implementazioni del cluster AEM Forms 6.5 LTS su JBoss EAP 8, i file domain/configuration/domain_oracle.xml, domain_mysql.xml e domain_mssql.xml possono contenere un tag `<security>` duplicato, con conseguente XML non valido che impedisce l’avvio del controller di dominio.
...
Gli utenti hanno riscontrato un danneggiamento dell’archivio durante la compattazione online se in precedenza era stata eseguita la compattazione offline nell’archivio JCR. In questo scenario può verificarsi un `SegmentNotFoundException` (SNFE). Questo può causare il danneggiamento dell’archivio.

Per risolvere il problema, installa l’hotfix tramite il gestore di pacchetti da [Distribuzione software](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/cq-6.5.lts.1-hotfix-GRANITE-65385-1.0.zip). Poiché l’hotfix include un bundle `oak-segment-tar` di basso livello, l’istanza viene riavviata dopo l’installazione.

Pianifica i tempi di inattività di un’istanza durante la sua applicazione. Per la compattazione offline, utilizza il [file jar](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar) eseguito da Oak corrispondente, disponibile anche in Distribuzione software.

>[!NOTE]
>
> Per qualsiasi operazione eseguita da Oak, utilizza il file jar [Oak-run 1.78.1-B006](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar) (collegato qui sopra).
...
### Bundle `com.adobe.granite.apicontroller` mancante in AEM 6.5 LTS SP1 (GRANITE-67639) {#missing-apicontroller-bundle-granite-67639}

Bundle `com.adobe.granite.apicontroller` mancante in AEM 6.5 LTS SP1. Questo bundle controlla come vengono risolti i bundle OSGi e può impedire la risoluzione dei bundle in altri bundle, il che è utile per limitare le API esposte.

Installa l&#39;aggiornamento rapido da [Distribuzione software](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/cq-6.5.lts.1-hotfix-GRANITE-67639-1.0.zip) per utilizzare questa funzionalità.
...
* [Elenco dei bundle OSGi inclusi in Experience Manager 6.5 LTS, Service Pack 1](/help/release-notes/assets/65lts_sp1_bundles.zip) <!-- UPDATE FOR EACH NEW RELEASE -->
* [Elenco dei Pacchetti di contenuti inclusi in Experience Manager 6.5 LTS, Service Pack 1](/help/release-notes/assets/65lts_sp1_packages.zip) <!-- UPDATE FOR EACH NEW RELEASE -->
