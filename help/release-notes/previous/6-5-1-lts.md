...
* **FORMS-24688:** Durante la configurazione di JBoss EAP 8.0 su Linux, gli script della shell modificati in Windows potrebbero contenere terminazioni di riga CRLF, causando errori `/bin/sh^M: bad interpreter` o $`\r`: comando non trovato. Per risolvere il problema, converte i file interessati in terminazioni di riga LF utilizzando dos2unix.

* **FORMS-24689:** In modalità di aggiornamento chiavi in mano, l&#39;aggiornamento della porta del database in `lc_turnkey.xml` potrebbe non essere applicato correttamente durante l&#39;aggiornamento. Il processo farà riferimento al valore della porta precedente.
