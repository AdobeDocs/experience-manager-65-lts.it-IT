...
>[!IMPORTANT]
>
> **Hotfix obbligatorio** - Per evitare problemi di SNFE (SegmentNotFoundException) con la compattazione offline durante l&#39;installazione del Service Pack 1, installare l&#39;hotfix descritto in [Problemi noti - Danneggiamento dell&#39;archivio durante la compattazione online](#repository-corruption-during-online-compaction-after-offline-compaction-granite-65146).
...
* Oracle Java™ 17/21 (64 bit) (Per ulteriori informazioni, consulta la sezione [Supporto Java](#java-support).
...
Per ulteriori informazioni, vedere **Migrazione da Javax a Jakarta Namespace**.
...
**Ambiente**
Applicabile a: clienti AEM 6.5 LTS (On-Premise) che installano Service Pack 1 (SP1). SP1 viene fornito come file Quickstart JAR.
...
* **FORMS-24688:** Durante la configurazione di JBoss EAP 8.0 (AEM Forms 6.5.1 LTS) su Linux, gli script creati o modificati in Windows possono contenere terminazioni di riga CRLF, causando errori /bin/sh^M: interprete non valido o $&#39;\r&#39;: comando non trovato. Assicuratevi che i file interessati utilizzino le terminazioni di riga LF. Questa operazione può essere eseguita utilizzando strumenti quali dos2unix o qualsiasi metodo equivalente.

* **FORMS-24689:** In modalità chiavi in mano, l&#39;aggiornamento della porta del database non viene applicato correttamente durante la nuova installazione o l&#39;aggiornamento. In modalità di installazione aggiornata, gli utenti possono selezionare solo tra le porte visualizzate sullo schermo. In modalità di aggiornamento, la nuova porta del database deve essere aggiornata manualmente in lc_turnkey.xml prima di procedere con l&#39;aggiornamento.

* **FORMS-24687:** Nelle distribuzioni del cluster AEM Forms 6.5 LTS in JBoss EAP 8, i file domain/configuration/domain_oracle.xml, domain_mysql.xml e domain_mssql.xml possono contenere un tag `<security>` duplicato, con conseguente XML non valido che impedisce l&#39;avvio del controller di dominio.
...
Gli utenti hanno riscontrato un danneggiamento dell’archivio durante la compattazione online se in precedenza era stata eseguita la compattazione offline nell’archivio JCR. In questo scenario può verificarsi un `SegmentNotFoundException` (SNFE). Può danneggiare l’archivio.

Per risolvere il problema, installare l&#39;aggiornamento rapido tramite Gestione pacchetti da [Distribuzione software](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/cq-6.5.lts.1-hotfix-GRANITE-65385-1.0.zip). Poiché l&#39;aggiornamento rapido include un bundle `oak-segment-tar` di basso livello, l&#39;istanza viene riavviata dopo l&#39;installazione.

Pianifica i tempi di inattività di un’istanza quando la applica. Per la compattazione offline, utilizza il [file jar](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar) corrispondente eseguito da Oak, disponibile anche in Distribuzione software.

>[!NOTE]
>
> Per qualsiasi operazione eseguita da Oak, utilizza il file jar [Oak-run 1.78.1-B006](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar) (collegato in alto).
