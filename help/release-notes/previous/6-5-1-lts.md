...
* **FORMS-24688:** Durante la configurazione di JBoss EAP 8.0 (AEM Forms 6.5.1 LTS) su Linux, gli script modificati in Windows possono contenere terminazioni di riga CRLF, causando errori /bin/sh^M: interprete non valido o $&#39;\r&#39;: comando non trovato. Converti i file .sh interessati in formato Unix (LF) utilizzando dos2unix (ad esempio, trova . -type f -name &quot;*.sh&quot; -exec dos2unix {} +) per risolvere il problema.

* **FORMS-24689:** In modalità di aggiornamento chiavi in mano, gli aggiornamenti alla porta del database non vengono selezionati dall&#39;interfaccia utente e devono essere modificati manualmente nel file di configurazione lc_turnkey.xml. Se non viene aggiornato correttamente, è possibile che il sistema continui a utilizzare il valore della porta precedente.

* **FORMS-24687:** Nelle distribuzioni del cluster LTS AEM Forms 6.5.1 in JBoss EAP 8, `domain/configuration/domain_oracle.xml` può contenere un tag `<security>` duplicato, che genera un XML non valido che impedisce l&#39;avvio del controller di dominio.
