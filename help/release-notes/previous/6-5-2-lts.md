...
* È stata aggiunta la richiesta della modalità rapida di Configuration Manager (LCM) per le credenziali di amministratore AEM durante la configurazione, sostituendo le credenziali predefinite incorporate. Questo miglioramento è disponibile tramite l’hotfix JEE SP2 6.5 LTS. (FORMS-26704)
...
**Sono stati risolti i seguenti problemi nell&#39;hotfix AEM Forms 6.5 LTS Service Pack 2 JEE (build 6.6.0-0006). Per applicarli, [scarica e installa l&#39;aggiornamento rapido](/help/forms/using/adobe-experience-manager-forms-65-lts-hotfixes.md).**

* Dopo l’aggiornamento di Apache Shiro alla versione 2.1.0, l’avvio di AEM Forms su JEE non è riuscito con un `NoClassDefFoundError` per il gestore della sicurezza di Shiro. Il bootstrap ora viene completato correttamente. (FORMS-26818)
* AEM Forms su JEE non riuscito con un errore &quot;nessuna classe trovata&quot; per `org.owasp.esapi.reference.JavaLogFactory`. La classe mancante è ora risolta. (FORMS-26819)
* Dopo l’aggiornamento ad AEM Forms 6.5 LTS, gli endpoint di TaskManager sono stati rimossi. Gli endpoint vengono ora mantenuti. (FORMS-26584, FORMS-26589)
* In JEE, il passaggio MergeEars di Configuration Manager non è riuscito con un errore di dichiarazione DOCTYPE (`ALC-LCM-010-200`) a causa del generatore XML protetto. Operazione MergeEars completata. (FORMS-26569)
* Registri a livello di applicazione mancanti nelle distribuzioni IBM® WebSphere® Liberty. La registrazione a livello di applicazione è ora ripristinata. (FORMS-25063)
* In JBoss®, e-mail non riuscita con &quot;IMAPProvider non è un sottotipo&quot;. L’e-mail ora funziona su JBoss®. (FORMS-24892)
* In WebSphere® Liberty Profile (WLP), l&#39;e-mail non è riuscita con &quot;Impossibile convertire il socket in TLS&quot;. L’e-mail su TLS ora funziona su WLP. (FORMS-24692)
* Aggiornamento della libreria Gibson alla versione 6.0.29665850. (FORMS-26688)
* Miglioramenti della convalida delle asserzioni SAML con backport. (FORMS-25222)
* Apache Log4j è stato aggiornato alla versione 2.25.5. (FORMS-26733, FORMS-26734)
* Questo hotfix include anche correzioni di sicurezza.
