...
# `We.Retail` Implementazione di riferimento{#we-retail-reference-implementation}

## Introduzione {#introduction}

Le pagine `We.Retail` sono un&#39;implementazione di riferimento e un contenuto di esempio che illustra il modo consigliato per impostare una presenza online con Adobe Experience Manager.

Il sito `We.Retail` utilizza le tecnologie Adobe Experience Manager (AEM) più recenti, ad esempio HTL, layout reattivi, modelli modificabili, componenti core e molto altro.
...
Come implementazione di riferimento standard di AEM, `We.Retail` presenta alcune delle funzionalità più potenti di AEM.

| **Funzione** | **Descrizione** | **Interessato?** |
|---|---|---|
| [Struttura del sito globalizzata](/help/sites-administering/tc-bp.md) | `We.Retail` include pagine in lingua primaria copiate in tempo reale in siti specifici del paese. | [Prova!](/help/sites-developing/we-retail-globalized-site-structure.md) |
...
| [Frammenti di contenuto](/help/assets/content-fragments/content-fragments.md) | Nella sezione Esperienze `We.Retail` è illustrata la possibilità di riutilizzare il contenuto tramite frammenti di contenuto. | [Prova!](/help/sites-developing/we-retail-content-fragments.md) |
...
Il sito `We.Retail` viene distribuito come contenuto di esempio di AEM. Per utilizzare, è sufficiente [avviare AEM come si farebbe normalmente](/help/sites-deploying/deploy.md#getting-started), assicurandosi che il contenuto di esempio non sia disabilitato.

>[!CAUTION]
>
>Non installare `We.Retail` nelle istanze di produzione. Le istanze di produzione devono essere avviate in `nosamplecontent` [modalità di esecuzione](/help/sites-deploying/configure-runmodes.md).

>[!CAUTION]
>
>Il sito `We.Retail` è basato sulla tecnologia AEM più recente e pertanto non supporta l&#39;authoring dell&#39;interfaccia utente [classica](/help/sites-classic-ui-authoring/classic-page-author-first-steps.md).

### Versione più recente {#latest-version}

Anche se `We.Retail` è distribuito con la versione di AEM, è possibile che gli aggiornamenti al contenuto e alle relative funzioni vengano apportati dopo la versione. È quindi possibile [scaricare l&#39;ultima versione da GitHub](https://github.com/Adobe-Marketing-Cloud/aem-sample-we-retail/releases), quindi [caricarla](/help/sites-administering/package-manager.md#uploading-packages-from-your-file-system) e [installarla](/help/sites-administering/package-manager.md#installing-packages) come pacchetto nell&#39;istanza AEM.

### Primi passaggi {#first-steps}
...
Geometrixx e le sue molte incarnazioni servivano come contenuto campione nelle versioni precedenti di AEM. A partire dalla versione 6.3, `We.Retail` è stato il contenuto di esempio consegnato con AEM e funge da nuova implementazione di riferimento standard.

Il sito `We.Retail` è tecnicamente più robusto e sfrutta la tecnologia AEM più recente per essere più flessibile e scalabile, dimostrando al contempo le funzionalità più recenti del prodotto.

### Confronto funzioni {#feature-comparison}

Nella tabella seguente viene fornita una panoramica delle funzioni principali disponibili in `We.Retail` rispetto a Geometrixx.

* **Disponibile** significa che nel contenuto di esempio sono presenti esempi della funzionalità.
* **Non disponibile** significa che il contenuto di esempio non contiene esempi di funzionalità, ma la funzionalità potrebbe essere ancora disponibile.


| **Funzione** | **`We.Retail`** | **Geometrixx** |
|---|---|---|
| Struttura globale del sito | Le pagine della lingua principale vengono copiate live nei siti specifici del paese | Non disponibile |
...
Il sito `We.Retail` è stato rilasciato come progetto open-source e la versione più recente del codice sorgente può essere scaricata da GitHub.
