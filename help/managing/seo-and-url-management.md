...
description: Scopri le best practice e le raccomandazioni SEO (Search Engine Optimization) per un’implementazione AEM.
topic-tags: managing
...
L’ottimizzazione SEO (Search Engine Optimization) è diventata una preoccupazione chiave per molti esperti marketing. Di conseguenza, i problemi relativi all’ottimizzazione SEO devono essere affrontati in molti progetti AEM.

In questo documento vengono innanzitutto descritte alcune [best practice per l&#39;ottimizzazione SEO](#seo-best-practices) e alcuni consigli relativi a un&#39;implementazione di AEM. Vengono quindi approfonditi alcuni dei [passaggi di implementazione più complessi](#aem-configurations) illustrati nella prima sezione.
...
La proprietà `sling:alias` ti consente di utilizzare anche Adobe cake. È possibile aggiungere `sling:alias` come proprietà a qualsiasi risorsa per consentire l&#39;utilizzo di un nome di alias per la risorsa. Nell’esempio precedente, si otterrebbe quanto segue:
...
Questo flusso consente agli strumenti di traduzione di AEM, come il gestore multisito, di continuare a mantenere una relazione tra:
...
Anche se questo approccio è documentato nella documentazione ufficiale di AEM e Sling, il supporto delle espressioni regolari fornito da questa implementazione ha un ambito limitato rispetto alle opzioni disponibili utilizzando direttamente `SlingResourceResolver`. Inoltre, una tale implementazione delle mappature può causare problemi con l’annullamento della validità della cache del Dispatcher.
...
1. **Regole di SlingResourceResolver**
   ...
   * **Apache Sling Resource Resolver Factory**
   ...
   Adobe consiglia di creare le mappature necessarie per ridurre gli URL come espressioni regolari, quindi di definire queste configurazioni in un nodo OsgiConfignode, `config.publish`, incluso nella build.
...
>[!NOTE]
>
>Disponibile come funzionalità prodotto a partire dalla versione 6.5.11.0 di Adobe Experience Manager.
> 
